...
document.addEventListener('DOMContentLoaded', () => {
    // 1. Дані про товари
    const productsData = [
        { id: 0, name: "1 дифузор + 2 змінні картриджі", price: 790, oldPrice: 990 },
        { id: 1, name: "2 дифузори + 4 змінні картриджі", price: 1490, oldPrice: 1980 },
    ...
    // 3. Селектори
    const cards = document.querySelectorAll('.order-pop-up__improvements__improvement');
    const basketContainer = document.querySelector('.order-pop-up__products'); 
    const totalPriceElement = document.querySelector('.order-pop-up__to-pay .new-price');
    ...
    function updateUI() {
        ...
        if (state.mainProductIndex !== null) {
            ...
            htmlContent += generateProductHTML(mainProd.name, mainProd.price, mainProd.oldPrice);
        }
        ...
        if (state.extraProductAdded) {
            ...
            htmlContent += generateProductHTML(`${extraProd.name} (x${state.extraQuantity})`, currentExtraPrice, currentExtraOldPrice);
        }

        // Рендер списку та фінальної суми
        basketContainer.innerHTML = htmlContent || '<div style="color: gray; padding: 10px 0;">Оберіть товари для замовлення</div>';
    ...
    cards.forEach((card, index) => {
        card.querySelector('.btn').onclick = () => {
            if (index < 2) {
                state.mainProductIndex = (state.mainProductIndex === index) ? null : index;
            } else {
